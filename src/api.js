  ...
  for (let i = 0; i < MAX_RETRIES; i++) {
    ...
    try {
      ...
      if (error.name === "AbortError") {
        console.log(
          `⏰ Request timed out after ${duration}ms (${DEFAULT_TIMEOUT / 1000}s limit)`,
        );
        lastErr = new Error(
          `Request timed out after ${DEFAULT_TIMEOUT / 1000}s - backend server may be slow or unresponsive`,
        );
      } else if (
        ...
        lastErr = new Error(
          `Cannot connect to backend server at ${API_BASE} - make sure it's running`,
        );
      } else {
      ...
      if (i < MAX_RETRIES - 1) {
        ...
        console.log(
          `🔄 Retrying in ${delay}ms (attempt ${i + 1}/${MAX_RETRIES})`,
        );
...
// FIX 3: moved to after safeFetch definition so it doesn't rely on hoisting
...
function normalizeResponse(data) {
  if (data && typeof data === "object" && "success" in data) {
    return {
      ...
      fetched_all: data.fetched_all || false,
    };
  }
...
export async function getPatientResources(
  ...
  try {
    ...
    // FIX 1: was requestCache.set(cacheKey, normalizedResponse, resourceParams) — args were swapped
    requestCache.set(cacheKey, resourceParams, normalizedResponse);
    return normalizedResponse;
  } catch (error) {
    console.error(
      `Error fetching ${resourceType} for patient ${patientId}:`,
      error,
    );
    return {
      success: false,
...
export async function fetchResources(resourceType, params = {}) {
  try {
    ...
    // FIX 2: was requestCache.set(cacheKey, params, normalizedResponse) — args were swapped
...
export async function fetchMeasurementTypes(params = {}) {
  try {
    ...
    // FIX 3: removed the 4th argument 600000 — set() only takes 3 params, it was silently ignored
    requestCache.set(cacheKey, measurementParams, normalizedResponse);
...
export async function getByIdDetailed(resourceType, id) {
  try {
    const response = await safeFetch(
      `/resources/${resourceType}/${id}/detailed`,
    );
...
export async function loadPatients(params = {}) {
  ...
  if (shouldUseAggregate(params.filters || {})) {
    try {
        ...
        page:
          Math.floor(
            (params._getpagesoffset || 0) /
              (params._count || CONFIG.ui.defaultPageSize),
          ) + 1,
      ...
      console.warn(
        "Aggregate flow failed, using traditional API:",
        error.message,
      );
      return await fallbackFn();
    }
 ...
 */
export async function loadPatientsWithFilters(
  params = {},
  appliedFilters = {},
) {
  const fallbackFn = async () => {
    try {
      ...
      if (params._getpagesoffset !== undefined)
        searchParams.append("_getpagesoffset", params._getpagesoffset);
      ...
      if (response.success) {
        ...
        console.log(
          `✅ Loaded ${response.data?.length || 0} filtered patients (${response.matching_patient_count} total matches)`,
        );
        return response;
      } else {
  ...
  if (shouldUseAggregate(appliedFilters)) {
    try {
      ...
      console.warn(
        "Aggregate flow failed for filtered patients, using traditional API:",
        error.message,
      );
      return await fallbackFn();
    }
...
/**
 * Cache implementation for request caching
 */
class RequestCache {
  constructor(
    maxSize = CONFIG.cache.maxCacheSize,
    ttlMs = CONFIG.cache.requestCacheTtl,
  ) {
    this.cache = new Map();
  ...
  _hashParams(params) {
    const sortedParams = Object.keys(params || {})
      ...
      .join("&");
    return btoa(sortedParams).replace(/[^a-zA-Z0-9]/g, "");
  }
...
export async function getFhirCapabilityStatement(serverUrl = null) {
  try {
    const params = serverUrl
      ? new URLSearchParams({ server_url: serverUrl })
      : "";
 ...
 */
export async function getBulkResourceSchemas(
  resourceTypes = null,
  serverUrl = null,
  sampleSize = 5,
) {
...
export async function clearBackendCache() {
  try {
    const data = await safeFetch("/resources/config/cache/clear", {
      method: "POST",
    });
    return data;
  } catch (error) {
...
export async function refreshDataAvailability() {
  try {
    const data = await safeFetch("/resources/data-availability/refresh", {
      method: "POST",
    });
    return data;
  } catch (error) {
